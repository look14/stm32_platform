...
u8 Stm32::SoftReset()
{
    ...
    if(COMM_CONTROL_ERR_OK != nRet)
        return nRet;
    
    /* Waiting usb removal */
    nRet = COMM_CONTROL_ERR_COMM;
    for(i=0; i<10000; i+=nStep, usleep(nStep*1000)) // 10s as timeout
    {
        if(FALSE==CommControl::GetInstance()->IsUsbReady()) {
    ...
    for(i=0; i<10000; i+=nStep, usleep(nStep*1000)) // 10s as timeout
    {
        if(CommControl::GetInstance()->ReopenUsb()) {
            nRet = COMM_CONTROL_ERR_OK;
            break;
        }
    }
