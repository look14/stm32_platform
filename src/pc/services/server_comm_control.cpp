...
BOOL CommControl::ReopenUsb()
{
	...
	return m_usbHid.Open(NULL, m_nUsbVendorId, m_nUsbProductId);
}
...
void CommControl::Init(void)
{
	...
	if(TRUE==ReopenUsb()){
		/*DWORD dwThreadID;
		if(INVALID_HANDLE_VALUE!=CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)CommControl::RecvThread, NULL, 0, &dwThreadID))
			return ;*/
...
u8 CommControl::PhySend(void* pParam, const u8 buf[], u16 len, u32 timeout)
{
	...
	for(i = 0; i < len; i += nMaxLen)
	{
		...
		if(!pCtrl->m_usbHid.Send(sendBuf, nMaxLen)) {
...
{  
	...
	u32 nTimeBeg = clock();
	while(m_nCommCtrlRecvDoneFlag==0 && clock()-nTimeBeg < timeout)
		Sleep(1);
...
unsigned int CommControl::RecvThread(LPVOID lpVoid)
{
	u8 tempBuf[1024];
	u16 i;
	u8  nRet;
	...
	CommControl* pCtrl = (CommControl*)lpVoid;

	while(true)
	{
		pCtrl->m_usbHid.Recv(tempBuf);
		if( pCtrl->m_pCommCtrlRecvBuf != NULL && 
			pCtrl->m_pCommCtrlRecvLen != NULL && 
			0!=pCtrl->m_usbHid.Recv(tempBuf))
		{
			for(i=0; i<tempLen; i++)
			{
				if(pCtrl->m_pCommCtrlRecvBuf == NULL || pCtrl->m_pCommCtrlRecvLen == NULL)
				...
				nRet = Comm_RecvBytes(&pCtrl->m_usbComm, tempBuf[i], pCtrl->m_pCommCtrlRecvBuf, pCtrl->m_pCommCtrlRecvLen);
				...
				//else if(SERVERCOMMPRO_ERR_RECV_REPEAT==nRet)
				//{
					//	printf("recv repeat\t");
				//	CommControl::SendRetry(1000);
				//	break;
				//}
			}
		}
