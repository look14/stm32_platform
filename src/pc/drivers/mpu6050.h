...
#define MPU6050_GET_S16(buf)	( (s16)( ((s16)(buf)[0] << 8) | (s16)(buf)[1] ) )
#define MPU6050_GRAVITY_ACCEL	( (double)9.788 )
...
public:
	...
	double GetUsTimeCounter();

public:
	u8 WriteRegMask(u8 addr, u8 dat, u8 mask);
	u8 WriteReg(u8 addr,u8 dat);
	u8 ReadReg(u8 addr, u8 *dat);
	u8 WriteNReg(u8 addr, const u8* pdat, u8 len);
	u8 ReadNReg(u8 addr, u8* pdat, u8 len);

	u8 GetDataU8(u8 addr);
	s16 GetDataS16(u8 addr);
	BOOL Config(void);
	BOOL GetCurrentData(double &, double &, double &, double &, double &, double &, double &);

protected:
	double m_fUsDivisor;
	double m_fGyroPrecision;
	double m_fAccelPrecision;
};
