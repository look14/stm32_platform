...
#define i2c_write(dev,addr,len,buf)   CommControl::GetInstance()->Mpu6050WriteRegs(addr, buf, len)//Sensors_I2C_WriteRegister
#define i2c_read(dev,addr,len,buf)    CommControl::GetInstance()->Mpu6050ReadRegs(addr, buf, len)//Sensors_I2C_ReadRegister 
#define delay_ms    Sleep//mdelay
#define get_ms(x)   *x = clock()//get_tick_count
...
struct gyro_reg_s reg = {0};
struct hw_s hw = {0};
struct test_s test = {0};
static struct gyro_state_s st = {0};
...
int mpu_config(void)
{
	...
	test.reg_gyro_fsr   = 0;    /* 250dps. */
	test.reg_accel_fsr  = 0x0;  /* Accel FSR setting = 2g. */
